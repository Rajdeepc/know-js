...
const NPMCard = ({ item }) => {
  return (
    ...
    <div className="npm-tags">
        ...
        return (<Nav.Item>
          <Nav.Link href={item.package && item.package.links && item.package.links.npm}>{itemKey}</Nav.Link>
        </Nav.Item>)
        })}
        </div>
         <Card.Text>Author: {item.package && item.package.author && item.package.author.name}</Card.Text>
      </Card.Body>
    </Card>
    </div>
  );
